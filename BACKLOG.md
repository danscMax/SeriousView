...
## M12 — Diagrams (Mermaid / PlantUML / Graphviz / …) · effort XL · ported · **DONE (Kroki, opt-in)**

**DONE 2026-06-13** (`4efc8af` + fix `6468d26`). Mermaid can't render natively (browser-only
`<foreignObject>`), so diagram fences render via the external **Kroki** service — the documented
"external service, gated opt-in" path. ```mermaid/```plantuml/```dot/… (any Kroki type, by fence
language) → `KrokiClient` POSTs to `{url}/{type}/{format}` → image (Mermaid→PNG, rest→SVG via
`Avalonia.Svg.Skia`). Core `DiagramTypes` + `KrokiClient`; preprocessor `ConvertDiagramFences`
(gated by `Transform(…, diagramsEnabled)`) → `::: diagram` container; `AdmonitionBlockHandler`
renders async (placeholder → image; in-memory cache; source+error fallback). **Strictly opt-in**
(`DiagramOptions`/`DiagramSettings`, default OFF, configurable URL incl. self-host — diagram text
leaves the machine) via Настройки ▸ Раскладка. No new deps (BCL HttpClient + the image-pipeline's
Avalonia.Svg.Skia). Verified live against kroki.io. **HTML-export of diagrams DONE**: when diagrams
are enabled, the exporter rewrites each diagram fence to `![type](krokiGetUrl)` — the browser fetches
the rendered image (a self-contained file can't run Mermaid's JS); disabled → the fence stays code.
GET payload = zlib-deflate→base64url (`Core/Text/KrokiUrl`); the fence walk is shared with the preview
pass via `MarkdownPreprocessor.WalkDiagramFences`. (Preview also gained a disk cache + render spinner +
click-to-lightbox.)
...
**Bigger ported milestones — now all DONE**: M12 diagrams (Kroki, opt-in — see the M12 section),
M13 export (HTML · rich-text · print/PDF via browser), M15 in-place editing + save. Remaining
deferred-with-reason: inline math `\(…\)` (no Markdown.Avalonia inline seam), paste-image &
spellcheck (Av12-gated).

**Beyond the port — additional native file formats DONE (2026-06-13)**: XML/NDJSON pretty-print +
TOML/INI/.env Ключ/Значение table (`2dca…` text-formats commit) · **in-app PDF viewer** (PDFtoImage
4.x / PDFium, page-by-page, fit-width + zoom, graceful "open externally" fallback) · **image viewer**
(raster PNG/JPEG/WebP/BMP/GIF/ICO via Skia + SVG via Avalonia.Svg.Skia). All route through
`FileLoadKind` before the binary classifier; PDF/image pin their Skia-based deps to the SkiaSharp
2.88.x line (Av11). See CLAUDE.md "Beyond the port".
...
reasons). **The full-port goal is COMPLETE** — and **M12 diagrams shipped 2026-06-13** (Kroki,
opt-in) along with extra native formats (PDF, images, XML/NDJSON/TOML). Only inline math `\(…\)`
(no inline-extension seam in Markdown.Avalonia) and the Av12-gated items remain deferred.

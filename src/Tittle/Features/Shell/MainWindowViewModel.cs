...
public partial class MainWindowViewModel : ViewModelBase, IDisposable
{
    ...
    {
        var target = await _fileDialog.SaveFileAsync("tittle-settings.json");
        if (target is null)
            return;
    ...
    /// <summary>Macros (M17) — recorder + library + replay, extracted to its own controller. Bound by the
    /// XAML menu (Macros.*) and the palette; the keyboard tunnel and DocumentView feed it intents.</summary>
    public MacroController Macros { get; }
    ...
    public IReadOnlyList<PaletteItem> BuildPaletteItems()
    {
        ...
        items.Add(new PaletteItem(Macros.IsRecordingMacro ? "Остановить запись макроса" : "Записать макрос",
            Macros.ToggleMacroRecordingCommand));
        items.Add(new PaletteItem("Управление макросами…", Macros.ShowMacroManagerCommand));
        if (Macros.HasMacro)
            items.Add(new PaletteItem("Воспроизвести последний макрос до конца файла", Macros.PlayMacroToEndCommand));
        foreach (var macro in Macros.Macros)
            items.Add(new PaletteItem($"Воспроизвести: {macro.Name}", Macros.PlaySavedMacroCommand, parameter: macro));
        foreach (var macro in Macros.Macros)
            items.Add(new PaletteItem($"Удалить макрос: {macro.Name}", Macros.DeleteMacroCommand, parameter: macro));
